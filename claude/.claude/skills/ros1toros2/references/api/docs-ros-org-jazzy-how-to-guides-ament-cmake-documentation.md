...
# ament_cmake user documentation — ROS 2 Documentation: Jazzy  documentation
...
Before using `ament_cmake`, it is very helpful to know the basics of CMake.
An official tutorial can be found here.

Basics

A basic CMake outline can be produced using `ros2pkgcreate<package_name>` on the command line.
...
`ros2pkgcreate` creates a package layout that follows these rules.
...
This is necessary to allow your library’s clients to use the `target_link_libraries(clientPRIVATEmy_library::my_library)` syntax.
...
Windows DLLs are treated as runtime artifacts and installed into the `RUNTIMEDESTINATION` folder.
...
The default working directory otherwise is the `CMAKE_CURRENT_BINARY_DIR`, which is described in the CMake documentation.
...
In principle, the ament index is contained in a folder within the install space.
...
When running `colconbuild`, this installs a file `my_rviz_displays` into a subfolder `rviz_common__pluginlib__plugin` into the resource_index.
...
The actual implementation of `ament_environment_hooks` can be found in the official ament-cmake repository.
...
Another example of using environment hooks for Gazebo plugin paths can be found in the official ros_gz_project_template.
...
These version headers are generated from the `package.xml` file using the ament_generate_version_header.cmake script.

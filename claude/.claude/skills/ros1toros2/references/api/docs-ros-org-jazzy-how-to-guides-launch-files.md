...
# Migrating Launch Files — ROS 2 Documentation: Jazzy  documentation
...
While launch files in ROS 1 are always specified using XML files, ROS 2 supports both XML and YAML files.
ROS 2 also supports Python launch scripts to enable more flexibility (see launch package).
...
A description of the ROS 2 launch system can be found in Launch System tutorial.
...
-
`eval` and `dirname` substitutions require escape characters for string values, e.g. `if="$(eval'\'$(varvariable)\'==\'val1\'')"`.
You can also use HTML escapes like `"` .

-
`eval` does not pass configurations ( `arg` ) as local Python variables.
They have to be accessed via `$(varname)`.
...
The rules that were shown in `Typeinferencerules` subsection of `param` tag applies to any attribute.
For example:

...
# Creating a launch file — ROS 2 Documentation: Jazzy  documentation
...
See Using XML, YAML, and Python for ROS 2 Launch Files for a description of the different formats.
...
Use `args` when mixing ROS and non-ROS arguments (e.g., `my_custom_arg--ros-args--log-levelinfo`), or `ros_args` for cleaner syntax with only ROS arguments like remappings, parameters, or log levels.
...
You learned about creating packages in Creating a package.
...
This helps make sure that the `ros2launch` command is available after building your package.
...
To see the system in action, open a new terminal and run the `ros2topicpub` command on the `/turtlesim1/turtle1/cmd_vel` topic to get the first turtle moving:
...
A hidden node (the `ros2topicpub` command you ran) is publishing data to the `/turtlesim1/turtle1/cmd_vel` topic on the left, which the `/turtlesim1/sim` node is subscribed to.
...
You can create launch files using XML, YAML, or Python, and run them using the `ros2launch` command.

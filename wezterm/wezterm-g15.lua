...
if wezterm.target_triple == "x86_64-pc-windows-msvc" then
	...
	table.insert(
		config.launch_menu,
		{ label = "PowerShell", args = { "C:/Program Files/PowerShell/7-preview/pwsh.exe", "-NoLogo" } }
	)
	table.insert(
		config.launch_menu,
	...
	table.insert(config.launch_menu, {
		...
		args = {
			"cmd.exe",
			"/k",
			"C:\\Users\\dengqi\\app\\VSIDE\\Common7\\Tools\\VsDevCmd.bat",
			"-startdir=none",
			"-arch=x64",
			"-host_arch=x64",
		},
	...
	table.insert(config.launch_menu, {
		...
		args = {
			...
			"-NoExit",
			"-Command",
			'&{Import-Module C:\\Users\\dengqi\\app\\VSIDE\\Common7\\Tools\\Microsoft.VisualStudio.DevShell.dll; Enter-VsDevShell b98a163a -SkipAutomaticLocation -DevCmdArguments "-arch=x64 -host_arch=x64"}',
		},
	})
	table.insert(config.launch_menu, {
		label = "MSYS zsh",
